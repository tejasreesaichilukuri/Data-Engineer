"""
print(1%12,end=" ")
print(2%12,end=" ")
print(3%12,end=" ")
print(4%12,end=" ")
print(5%12,end=" ")
print(6%12,end=" ")
print(7%12,end=" ")
print(8%12,end=" ")
print(9%12,end=" ")
print(10%12,end=" ") 
print(11%12,end=" ")
print(12%12,end=" ")
print(13%12,end=" ")
print(14%12,end=" ")
print(15%12,end=" ")
print(16%12,end=" ")
print(17%12,end=" ")
print(18%12,end=" ")
print(19%12,end=" ")
print(20%12,end=" ")
print(21%12,end=" ")
print(22%12,end=" ")
print(23%12,end=" ")
#print( %12,end=" ")
print(24%12,end=" ")
print(25%12,end=" ")
print(26%12,end=" ")
print(27%12,end=" ")
print(28%12,end=" ")
print(29%12,end=" ")
print(30%12,end=" ")
"""
"""
Marks=int(input("Enter your marks: "))
if(Marks >= 36 and Marks <= 100):
    print("you are pass")
else:
    print("failed")
"""

#check the both number given are positive and atleast one of the number is greater than 10?
"""
num1=int(input("Enter number1: "))
num2=int(input("Enter number2: "))
positive= num1 > 0 and num2 > 0
greater = num1 > 10 or num2 > 10
Result= positive and greater
print(Result)
"""
#you have 3users x,y,z  electricity bills check the sum of any two users should be always greater than 3bill?
x=int(input("Enter the Electricity bill amount: "))
y=int(input("Enter the Electricity bill amount: "))
z=int(input("Enter the Electricity bill amount: "))
condition= x+y > z and y+z > x and z+x > y
print(condition)